...
% interest (ROI) by drawing a polygon. It will create a .mat
...
% Updated 2023-02-27, AHS: incorporate GUI function, allow for choice of
% drawing style
...
colorq = input("Visualize particles using displacement or cell stain? (0: R map, 1: stain map) "); 
drawq = input("Draw Style? (0: Freehand, 1: Multiple Clicks) "); 
if drawq == 0
    drawstyle = 'freehand';
elseif drawq ==1
    drawstyle = 'click';
else
    error('Please specify draw style correctly (0: Freehand, 1: Multiple Clicks)')
end
...
elseif colorq ==1
    ...
    error('Please specify color correctly (0: R map, 1: stain map)')
end
...
for i = 1:length(file_list)
   ...
   load(file_name);
   roidata = struct('label',{},'position',{});
   
   
   if ~exist('iMP','var'); iMP = 1; end
   for k = 1:length(MPStats)
       roi = selectROI_GUI(MPStats(k),color,drawstyle);
       roidata(k).position = roi.position;
       roidata(k).label = MPStats(k).FileName;
       close
   end
